...
**Adapter pattern**. Each extension point implements a common interface defined in [MetricsInterface](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/metrics/base.py), [ModelInterface](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/models/base.py), and [StorageInterface](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/storage/base.py). The core engine calls interface methods without knowing which specific adapter is behind them. This keeps the core decoupled from implementation details. To add a new adapter, implement the relevant interface and register with its registry ([metrics](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/metrics/factory.py), [models](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/models/factory.py), or [storage](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/storage/factory.py)). All three use decorator-based self-registration, so there's no central file to modify.

**Data contracts**. The [Schema system](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/core/contracts.py) maps external column names to the engine's standard schema. Each data source defines its mappings once in a single place. The contract handles translation automatically, and the rest of the system works unchanged.

---

## Data Flow

The system is **configuration-driven**. A single config file selects which adapters to use, and data flows through four stages orchestrated by [engine.py](https://github.com/eisenhauerIO/tools-impact-engine-measure/blob/main/impact_engine_measure/engine.py).
